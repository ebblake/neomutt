...
/**
 * mutt_qsort_r - Sort an array, where the comparator has access to
 * opaque data rather than requiring global variables.  Note that this
 * implementation might not be re-entrant: signal handlers and @compar
 * must not call mutt_qsort_r.
 ...
 * @param arg    Opaque argument to pass to @compar
 */
void mutt_qsort_r(void *base, size_t nmemb, size_t size, qsort_r_compar_t compar, void *arg)
{
#ifdef HAVE_QSORT_S
  /* Older FreeBSD, where qsort_r had incompatible signature */
